...
def q5(client):
    ...
    q = """
            ...
            2. total_avg
            select avg(like_num) from `w4111-columbia.graph.tweets`
            ...
            (select twitter_username as node from `w4111-columbia.graph.tweets`
            group by twitter_username having avg(like_num) >=
            (select avg(like_num) from `w4111-columbia.graph.tweets`)) as T
            where G.node = T.node
...
def pre_pagerank(client):
    ...
    q = """
        INSERT INTO dataset.Pagerank(node, outdeg) 
        select src, count(*) from dataset.GRAPH group by src
        """
    job = client.query(q)
    results = job.result()

    q = """
        UPDATE dataset.Pagerank SET outdeg = 0 where outdeg IS NULL
        """
    job = client.query(q)
    results = job.result()
